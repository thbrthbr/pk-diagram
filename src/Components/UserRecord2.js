...
const UserRecord2 = () => {
  ...
  const [tierline, setTierLine] = useState('');
  ...
  const backMobile = (id) => {
    ...
    for (let i = 0; i < imgSet.length; i++) {
      if (i == index1) {
        ...
        if (imgSet[i].id !== selectedItem.id) {
          if (selectedItem.id !== id) renew.push(imgSet[i]);
        }
      }
    }
    setTierLine('');
...
const $TitleLine = styled.textarea`
  font-family: 'wehaven-regular';
  font-style: bold;
  font-size: 30px;
  padding: 0;
